...
# FACTOR 2: RSI → 倉位權重調節
# ═══════════════════════════════════════════
# RSI < 30: 超賣，加重首批；RSI 30-45: 偏弱，正常比重；RSI > 45: 未超賣，降低首批
if rsi < 25:
...
# FACTOR 3: 多週期共振 → 積極度
# ═══════════════════════════════════════════
# tf_verdict: "bullish", "bearish", "mixed", or None
...
if pb < 3:
    ...
    # Base allocation (均衡型: 30/30/40)
    base_alloc = [30, 30, 40]
    
    # Apply RSI boost to first batch
    ...
    # Phase 1: Nearest support or current price if already at support
    s1_name, s1_price = unique_supports[0]
    dist_to_s1 = (p - s1_price) / p * 100
    
    if dist_to_s1 < 2:
        # Already at or very near first support
    ...
    # Phase 3: Confirmation rally or third support
    if len(unique_supports) >= 3 and aggression < 1.0:
        # Bearish: use third support as cautious entry
        ...
        # Neutral/Bullish: confirm reversal then add
    ...
    # 支撐位不足 → fallback 到比例式，但用 ATR 調節間距
...
# ═══════════════════════════════════════════
# STOP LOSS — 必須低於所有進場價位
# ═══════════════════════════════════════════
# 找出計畫中最低的進場價
entry_prices = [x["pt"] for x in phases if x["pct"] > 0 and x["pt"] > 0]
lowest_entry = min(entry_prices) if entry_prices else p
...
if unique_supports:
    ...
    s1 = lowest_support - atr
    if s1 < lowest_entry:
        stop_candidates.append(("最低支撐下方 1×ATR", s1))

# 候選 2: 200MA 下方 1 ATR（只在 200MA 低於最低進場價時有意義）
if ma200 and ma200 > 0 and (ma200 - atr) < lowest_entry:
    stop_candidates.append(("200MA 下方 1×ATR", ma200 - atr))

# 候選 3: 最低進場價再往下 1.5 ATR（保底邏輯）
fallback_stop = lowest_entry - atr * 1.5
stop_candidates.append(("最低進場價下方 1.5×ATR", fallback_stop))

# 絕對底線: 不低於 -20%（避免停損設太遠）
absolute_floor = p * 0.80

# 選擇最高的候選（最保守的停損），但不能高於最低進場價，也不能低於絕對底線
stop_candidates.sort(key=lambda x: -x[1])  # 由高到低
stop_why, stop_price = stop_candidates[0]
stop_price = max(stop_price, absolute_floor)  # 不低於 -20%
stop_price = min(stop_price, lowest_entry * 0.97)  # 必須至少低於最低進場價 3%
...
# Step plan — compute tf_verdict for this ticker
